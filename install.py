#!/usr/bin/python
# -*- coding: latin-1 -*-
# Copyright 2021 by Adriano Sanna. All Rights Reserved.
#
"""
...
global scriptPath
global dest
#get version and script path
versionNow= mc.about(version=True)
scriptPath = "maya/"+versionNow+"/scripts"
dest=''
...
def _onMayaDropped():
    ...
    path2copy= os.path.join(srcPath, 'asGep')
    ...
    for path in sys.path:
        if path.endswith(scriptPath):
            print ("sys.path for script is:" + path)
            dest=path
    if os.path.exists(dest + '/asGep/asGep.py'):
        maya.cmds.warning('AS Gep is already installed at ' + dest+'/asGep')
        shutil.rmtree(dest+'/asGep', ignore_errors=False)
        maya.cmds.warning('Remove older version')
        copyDirectory(path2copy, dest+'/asGep')
        print('>>>source:' + path2copy)
        print('>>>destination:' + dest)
        mc.getFileList( folder=mc.internalVar(userScriptDir=True) )
    ...
    iconPath = os.path.join(dest, 'asGep', 'icons', 'asgep.png')
    print(iconPath)
...
mel.eval('glmSimulationCacheLayoutCmdWithFocus()') #open layout editor windows
...
asToolPath= userAppDir+'2019/scripts/asGep/'
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def create_connections(self):
        ...
        self.ui.refreshLoadButton.clicked.connect(self.loadRefreshData) # refresh entity data (preset + selection) on load tab
        self.ui.loadLayoutFileButton.clicked.connect(self.loadLayoutFile) #load layout file
        self.ui.importToLayoutButton.clicked.connect(self.createLayoutNode) #import data from preset
...
class XmlEntityData(object):
    ...
    def getAllData(self,preset):
        ...
        self.getEntityItemListFromXml(root,self.blindDataList,'bditm','name')#3
    ...
    def clear(self):
        del self.meshesList[:]
        del self.shadingGroupsList[:]
        del self.shaderAttrList[:]
        del self.blindDataList[:]
        self.presetFile = ''
        self.gchaFile = ''
...
print("\n// asGep has been added to current shelf.")
